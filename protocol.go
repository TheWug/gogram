...
func (this *Protocol) BuildEditMessageMediaReq(o data.OMediaEdit) (reqtify.Request) {
	req := this.client.New("editMessageMedia").Method(reqtify.POST).
			   ArgDefault("chat_id", o.SourceChatId, "0").
			   ArgDefault("message_id", o.SourceMessageId.String(), "0").
			   ArgDefault("inline_message_id", o.SourceInlineId.String(), "")
	if o.ReplyMarkup != nil {
		b, e := json.Marshal(o.ReplyMarkup)
		if e != nil { return nil }
		req.Arg("reply_markup", string(b))
	...
	if o.Media != nil {
		serializer := struct {
			...
			Type: o.Media.InputMediaType(),
		}
		applyFileJson(req, &serializer.Media, o.Media.InputMedia(), 0)
		applyFileJson(req, &serializer.Thumb, o.Media.InputThumb(), 1)

		// this is terrible, and i know it, but it works so i'll fix it later.
		// HACK XXX TODO FIXME
		b1, e := json.Marshal(o.Media)
		if e != nil { return nil }
		b2, e := json.Marshal(serializer)
		if e != nil { return nil }
		b1[len(b1)-1] = byte(',')
		b1 = append(b1, b2[1:]...)
		req.Arg("media", string(b1))
	} else {
		return nil
	}
...
// this resolves the allowed file attachment modes for a file object inside a serialized json chunk.
// differs from applyFile, which is for file objects directly included as parameters.
// object can be any of the following:
// FileID: passed directly to API, represents an internal (already uploaded) file.
// string: passed directly to API, assumed to be a URL from which telegram will download the file.
// io.Reader: marshalled into a form file, where it will be read to completion by the HTTP request. filename is autogenerated.
// byte array: same as above. filename is autogenerated.
// reqtify.FormFile: same as above, but uses included filename.
func applyFileJson(req reqtify.Request, parameter *string, object interface{}, serial int) {
	if object == nil { return }
	param_name := fmt.Sprintf("attach://attachment_%d", serial)
	switch file := object.(type) {
	case data.FileID:
	...
	case io.Reader:
		*parameter = param_name
		req.Method(reqtify.POST).FileArg(param_name[9:], param_name[9:], file)
	case []byte:
		*parameter = param_name
		req.Method(reqtify.POST).FileArg(param_name[9:], param_name[9:], bytes.NewReader(file))
	case reqtify.FormFile:
		*parameter = param_name
		req.Method(reqtify.POST).FileArg(param_name[9:], file.Name, file.Data)
	default:
		panic("unsupported file attachment mode")
	}
}
...
func (this *Protocol) EditMessageMediaAsync(o data.OMediaEdit, sm data.ResponseHandler) () {
	go DoAsyncCall(this.bot.Log, this.BuildEditMessageMediaReq(o), sm)
}
...
func (this *Protocol) EditMessageMedia(o data.OMediaEdit) (*data.TMessage, error) {
	var m data.TMessage
	j, e := DoCall(this.bot.Log, this.BuildEditMessageMediaReq(o))
	return &m, OutputToObject(j, e, &m)
}
